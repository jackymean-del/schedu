...
function AndGroupCard({
  ...
  const setCount = (secName: string, bundleId: string, val: number) => {
    onUpdate({
      ...group,
      aiSuggested: false,  // user edit clears AI badge
      strengthMatrix: {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ fontSize: 13, fontWeight: 800, color: '#13111E', flex: 1, display: 'flex', alignItems: 'center', gap: 7 }}>
          {group.aiSuggested && (
            <span style={{
              fontSize: 9, background: '#7C6FE0', color: '#fff',
        ...
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
          {group.bundles.map(b => (
            <span key={b.id} style={{
              ...
              background: b.color ?? '#7C6FE0', color: '#fff', borderRadius: 4,
            }}>
          ...
          <table style={{ borderCollapse: 'collapse', width: '100%' }}>
            <thead>
              <tr>
                ...
                <th style={miniTH('center', 70)}>Total</th>
                {group.bundles.map(b => (
          ...
          {group.generatedGroups && group.generatedGroups.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {group.generatedGroups.map(g => {
                const bundleColor = group.bundles.find(b => b.id === g.bundleId)?.color ?? '#EDE9FF'
                return (
                  <span key={g.id} title={g.sectionSlices.map(s => `${s.sectionName}: ${s.studentCount}`).join(', ')} style={{
                    fontSize: 10, fontWeight: 700, padding: '3px 9px',
                    background: bundleColor, color: '#fff', borderRadius: 5,
                  }}>
...
function AndGroupModal({
  ...
  const toggleBundleSubject = (bundleId: string, subj: string) => {
    setBundles(bs => bs.map(b => {
      ...
      return {
        ...b,
        subjects: b.subjects.includes(subj)
