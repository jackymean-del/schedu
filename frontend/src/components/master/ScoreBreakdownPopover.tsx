    ...
    <span style={{ position: 'relative' as const, display: 'inline-flex' }}>
      ...
      >
        ...
        {liveScore !== originalScore && (
          <span style={{ color: improved ? '#0A8136' : '#DC2626', fontWeight: 800, fontFamily: "'DM Mono', monospace" }}>
            ({delta > 0 ? '+' : ''}{delta})
          </span>
        )}
          ...
          >
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                ...
                <span style={{ fontSize: 11, color: '#6D6A8A' }}>
                  total penalty points
                </span>
                {liveScore !== originalScore && (
                  <span style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 700, color: improved ? '#0A8136' : '#DC2626' }}>
            ...
            <div style={{ padding: '10px 14px 14px', maxHeight: 340, overflowY: 'auto' as const }}>
                      ...
                      return (
                        <div key={b.constraint}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                            ...
                            <span style={{ fontSize: 10, color: '#6D6A8A' }}>
              ...
              {tab === 'trend' && (
                !trendData || trendData.length === 0 ? (
                  <div style={{ padding: '20px 0', textAlign: 'center' as const, color: '#6D6A8A', fontSize: 12 }}>
                    Apply at least one fix to see constraint trends.
                  </div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 10 }}>
                    <div style={{ fontSize: 10, color: '#6D6A8A', marginBottom: 2 }}>
                    ...
                    {trendData.map(row => {
                      ...
                      return (
                        <div key={row.constraint} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          ...
                          <ConstraintSparkline
                            values={row.values}
                            color={resolved ? '#0A8136' : c.fg}
                          />

                          {/* First → Last */}
                          <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, flexShrink: 0 }}>
                            <span style={{ fontFamily: "'DM Mono', monospace", color: '#6D6A8A' }}>{row.first}</span>
                            <span style={{ color: '#D8D2FF' }}>→</span>
                            <span style={{
                              fontFamily: "'DM Mono', monospace", fontWeight: 800,
                              color: resolved ? '#15803D' : improved ? '#0A8136' : worsened ? '#DC2626' : '#6D6A8A',
                            }}>{row.last}</span>
                          </div>
              ...
              {/* Footer hint */}
              <div style={{
                fontSize: 10, color: '#6D6A8A', marginTop: 12,
