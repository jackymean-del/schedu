...
export function InstitutionHeader({ title, session }: { title: string; session?: string }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, borderBottom: '2px solid #685DBC', paddingBottom: 10, marginBottom: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div>
          ...
          {inst.address && <div style={{ fontSize: 10.5, color: '#6D6A8A', marginTop: 2, maxWidth: 360 }}>{inst.address}</div>}
        </div>
      </div>
      <div style={{ textAlign: 'right', flexShrink: 0 }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: '#685DBC' }}>{title}</div>
        {session && <div style={{ fontSize: 11, color: '#6D6A8A', marginTop: 2 }}>Session {session}</div>}
      </div>
    </div>
  )
}
    ...
    <div className="schedu-print-footer" style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, marginTop: 10, paddingTop: 6, borderTop: '1px solid #EEE' }}>
      {!inst.isPaid ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, opacity: 0.5 }}>
          <div style={{ width: 18, height: 18, borderRadius: 5, background: '#685DBC', display: 'flex', alignItems: 'center', justifyContent: 'center', WebkitPrintColorAdjust: 'exact' as any }}
            dangerouslySetInnerHTML={{ __html: SCHEDU_MARK }} />
          <div style={{ fontSize: 8.5, color: '#6D6A8A' }}>Generated by sched<span style={{ color: '#685DBC', fontWeight: 700 }}>U</span> · schedu.bhusku.com</div>
        </div>
      ) : <span />}
    ...
    <div className="schedu-print-overlay" {...dialogProps}>
      <div className="schedu-print-toolbar no-print">
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          {subtitle && <span style={{ fontSize: 12, color: '#6D6A8A' }}>{subtitle}</span>}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <label title="Pack as much onto each page as fits (saves paper). Off = one block per page."
            ...
            <input type="checkbox" checked={paperSaving} onChange={e => setPaperSaving(e.target.checked)} style={{ width: 16, height: 16, accentColor: '#685DBC', cursor: 'pointer' }} />
            🌱 Paper-saving mode
          </label>
          <button onClick={doPrint}
            style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '11px 28px', borderRadius: 10, border: 'none', background: '#685DBC', color: '#fff', fontSize: 15, fontWeight: 700, cursor: 'pointer', boxShadow: '0 4px 14px rgba(124,111,224,0.4)' }}>
            🖨️ Print
          </button>
